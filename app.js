...
function selectWizardService(service) {
    ...
    if (service.category === 'pelo') {
        ...
        // Limpiar manejadores previos clonando los botones
        ...
        // Pasar automáticamente al paso 2
        setTimeout(() => goToStep(2), 250);
    }
}
// Paso 2: Renderizar Barberos en el Wizard
function renderWizardBarbers() {
    wizardBarbersList.innerHTML = '';
    BARBERS.forEach(barber => {
        const card = document.createElement('div');
        card.className = 'barber-card';
        card.innerHTML = `
            <div class="barber-avatar-simulated">${barber.avatarInitials}</div>
            <h5>${barber.name}</h5>
        ...
        card.addEventListener('click', () => {
            activeBooking.barber = barber;
            document.querySelectorAll('.barber-card').forEach((el, idx) => {
                if (BARBERS[idx].id === barber.id) {
                    el.classList.add('selected');
                } else {
                    el.classList.remove('selected');
                }
        ...
        wizardBarbersList.appendChild(card);
    });
}
// Paso 3: Calendario e Interconectividad de Fechas y Horas
function renderCalendar() {
    ...
    const totalDays = new Date(year, month + 1, 0).getDate(); // Cantidad de días del mes
    
    // Obtener fecha de hoy para deshabilitar fechas pasadas
    ...
    for (let day = 1; day <= totalDays; day++) {
        ...
        // Deshabilitar domingos (cerrado) y días pasados
    ...
    // Si ya hay fecha seleccionada en este mes al renderizar
    if (activeBooking.date && activeBooking.date.getMonth() === month && activeBooking.date.getFullYear() === year) {
...
function renderTimeSlots(date) {
    ...
    // Lista de horarios del local (Lunes a Viernes: 12:00 a 20:30 hs)
    const baseHours = ["12:00", "13:00", "14:00", "15:00", "16:00", "17:00", "18:00", "19:00", "20:00"];
    
    // Sábados: 10:00 a 20:30 hs
    const isSaturday = date.getDay() === 6;
    const hours = isSaturday ? ["10:00", "11:00", "12:00", "13:00", "14:00", "15:00", "16:00", "17:00", "18:00", "19:00", "20:00"] : baseHours;
    
    // Simular que algunos turnos ya están reservados aleatoriamente para simular tiempo real
    // Usamos el día como semilla para que sea determinista en el día
    ...
    hours.forEach((time, index) => {
        ...
        // Simulación: deshabilitar 3 turnos al azar por barbero
        const isBooked = (seed * (index + 1) * 7) % 5 === 0;
        
        // Deshabilitar turnos del día de hoy en horas ya transcurridas
        const now = new Date();
...
// Paso 4: Cargar resumen en el formulario final
...
// Formulario de Reserva - Guardado y simulación de WhatsApp
function handleBookingSubmit(e) {
    ...
    const newBooking = {
        ...
        date: activeBooking.date.toISOString(), // Guardar string serializable
    ...
    // Generar mensaje de confirmación por WhatsApp — llega directo a Luciano
    const dateOptions2 = { weekday: 'long', day: 'numeric', month: 'long' };
    const readableDate = activeBooking.date.toLocaleDateString('es-ES', dateOptions2);
    const serviceType = activeBooking.service.category === 'barba' ? 'Barba' : activeBooking.service.title.toLowerCase().includes('barba') ? 'Corte/Barba' : 'Corte';
    ...
    // Ir a pantalla de ticket de éxito
    goToStep('success');
...
function renderBookingsList() {
    ...
    if (bookings.length === 0) {
        ...
        `;
        // Volver a enlazar el trigger porque se reescribe el HTML
    ...
    // Mostrar de más nuevos a más viejos
...
// ==========================================
// Buscar producto por ID (simple o variante)
function findProductById(id) {
    for (const prod of PRODUCTS) {
        ...
        if (prod.variants) {
            ...
            if (variant) {
                // Retornar objeto de producto combinado para compatibilidad con el carrito
                return {
                    id: variant.id,
                    title: variant.title,
                    price: prod.price,
...
function addToCart(prodId, qty = 1) {
    ...
    // Efecto de apertura del carrito automático al agregar producto
    cartPanel.classList.add('open');
}
...
function handleCartCheckout() {
    ...
    messageText += `\n💰 *Total a pagar:* *$${total}*\n\n_Coordinamos el retiro por el local en O'Higgins 179._`;
...
// Configurar los controles del modal de variantes al cargar la página
document.addEventListener('DOMContentLoaded', () => {
